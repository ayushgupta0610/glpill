...
## (a) YouTube Shorts — titles, descriptions, tags
Upload the `marketing/reels/` files as Shorts (accessible from India, no warmup needed). YouTube = search, so titles are keyword-rich. Reuse the same trending-audio-in-app idea, or keep original audio for Shorts (YT is fine either way).

**Shorts title formula:** `[hook / benefit] | GLP-1 pill tip` — front-load the keyword.

| Reel file | Short title | Description (paste) |
|---|---|---|
...
### Reddit (post at launch, after 2–3 weeks of genuine comment history)
Check each sub's self-promo rules first; DM mods if unsure. Post to r/Semaglutide, r/Rybelsus (if exists), r/GLP1, and orforglipron/Foundayo threads. One honest builder post, not copy-paste spam:

> **Title:** I got frustrated that every GLP-1 tracker is built for injections, so I made one for the pills
>
> Hey all — I'm on the oral-GLP-1 side (the pills), and every tracking app I tried is really an injection app with pills bolted on: "shot day," injection sites, weekly cadence. The daily pill is a different routine — especially Rybelsus with the empty-stomach + 30-minute wait, which I kept messing up.
>
